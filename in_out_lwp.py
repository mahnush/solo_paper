...
var_name_model = ['nd', 'lwp_dw', 'tau', 'reff']
...
def get_in_out(i):
    ...
    control_var_time_step = postpro_func.read_nc(model_con[i], var_name_model[1])[:, :]*1e3
    perturbed_var_time_step = postpro_func.read_nc(model_per[i], var_name_model[1])[:, :]*1e3
    control_var = control_var_time_step
    #control_var = np.ma.mean(control_var_time_step, axis = 0)
    perturbed_var = perturbed_var_time_step
    #perturbed_var = np.ma.mean(perturbed_var_time_step, axis = 0)
...
axs0.grid(True)
axs1.grid(True)
plt.savefig('pdf_lwp_no_legend.png')
plt.savefig('pdf_lwp_no_legend.pdf')
