...
model_con_10Apr = '/home/mhaghigh/solforia/new_version_run/new_ccn_all_data_per/per_qnc.nc'


var_name_model = ['qnc', 'lwp', 'tau', 'reff']
var_name_modis = ['nd', 'lwp', 'tau', 're']



control_nd = postpro_func.read_nc(model_con_10Apr, var_name_model[0])
re_modis = postpro_func.read_nc(modis_10Apr, var_name_modis[0])
control_nd = control_nd[0, 60:73, :, :]*1e-6
control_nd_mean = np.ma.mean(control_nd, axis=0)
#control_nd_mean = control_nd
#print(np.shape(control_nd_mean))
...
postpro_func.visulize_sat(ax0, re_modis, lat_modis , lon_modis , nd_bound, cbar_label[0],
 titel_kind[0], limit)
#2end subplot
postpro_func.visulize_model(ax1, control_nd_mean, nd_bound, cbar_label[0], titel_kind[1], limit)
plt.show()

re_modis_1dim = re_modis.flatten()
re_modis_1dim_n = re_modis_1dim[re_modis_1dim >5.]
re_modis_1dim_c = re_modis_1dim_n.compressed()
control_nd_mean_1dim = control_nd_mean.flatten()
